...
const EventCard: React.FC<EventCardProps> = ({
  ...
  return (
    <div
      className={cn(
        "absolute border-l-4 rounded-r shadow-sm",
        "cursor-pointer select-none transition-all duration-150",
        "hover:shadow-md hover:brightness-110",
        "border border-opacity-20 border-white", // Borde sutil para separación visual
        isShortEvent ? "px-2 py-0 text-xs flex items-center" : "px-2 py-1 text-xs",
    ...
    >
      {isShortEvent ? (
        // Layout for short events (30 minutes or less)
        <div className="flex items-center justify-between h-full">
          <div className="font-medium truncate text-xs" style={{ lineHeight: '1.1' }}>
            {event.title}
          </div>
          {!event.allDay && (
            <div className="text-xs opacity-90 flex-shrink-0 ml-1" style={{ lineHeight: '1.1' }}>
              {formatDate(event.start, 'HH:mm')}-{formatDate(event.end, 'HH:mm')}
            </div>
          )}
        </div>
      ) : (
        // Layout for normal events (more than 30 minutes)
        <>
